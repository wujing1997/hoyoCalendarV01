...
class AgentSession(Base):
    """Shared short-term agent conversation history.

    Persisted to PostgreSQL so that consecutive requests for the same
    (user_id, session_id) keep adjacent context even across uvicorn workers or
    a process restart. `messages` only stores assistant/user turns needed for
    context; it is never written to logs.
    """

    __tablename__ = "agent_sessions"
    __table_args__ = (
        UniqueConstraint("user_id", "session_id", name="uq_agent_sessions_user_session"),
    )
    ...
    messages: Mapped[Optional[list]] = mapped_column(JSONB, nullable=False, default=list)
    state: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=False, default=dict)
...
class RateLimitEvent(Base):
    """Shared sliding-window rate-limit events across API workers."""
