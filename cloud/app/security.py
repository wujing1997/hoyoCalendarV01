...
def get_client_ip(request) -> str:
    """Trust forwarded client IPs only when the direct peer is configured."""
...
def allow_request_db(db, key: str, limit: int, window_seconds: int) -> bool:
    """Record a shared, race-free sliding-window hit in PostgreSQL."""
    ...
    db.execute(
        select(func.pg_advisory_xact_lock(_advisory_lock_id("rate-limit", key)))
    )
    cutoff = utcnow() - timedelta(seconds=window_seconds)
    db.execute(
        delete(models.RateLimitEvent).where(
            models.RateLimitEvent.bucket_key == key,
            models.RateLimitEvent.created_at < cutoff,
        )
    )
    count = db.execute(
        select(func.count())
        .select_from(models.RateLimitEvent)
        .where(
            models.RateLimitEvent.bucket_key == key,
            models.RateLimitEvent.created_at >= cutoff,
        )
    ...
    if count >= limit:
        db.commit()
        return False
    db.add(models.RateLimitEvent(bucket_key=key, created_at=utcnow()))
    db.commit()
    return True
