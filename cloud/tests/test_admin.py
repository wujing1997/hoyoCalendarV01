...
def _admin_token(client):
    resp = client["admin"].post("/api/admin/login", json={
        "username": "admin",
...
def test_admin_routes_not_exposed_on_public_app(client):
    for path in ("/api/admin/users", "/api/admin/invites", "/api/admin/usage", "/api/admin/audit"):
    ...
    resp = client["admin"].get("/api/admin/users", headers=auth_headers("bogus"))
...
def test_invite_created_and_usable(client):
    ...
    listed = client["admin"].get("/api/admin/invites", headers=auth_headers(token))
...
def test_revoke_invite_blocks_use(client):
    token = _admin_token(client)
    created = client["admin"].post("/api/admin/invites", json={}, headers=auth_headers(token))
    invite_id = created.json()["id"]
    code = created.json()["code"]
    revoked = client["admin"].delete(f"/api/admin/invites/{invite_id}", headers=auth_headers(token))
...
def test_disable_user_revokes_sessions(client):
    ...
    user_id = client["api"].get("/api/auth/me", headers=auth_headers(user["access_token"])).json()["id"]
    ...
    assert client["api"].get("/api/auth/me", headers=auth_headers(user["access_token"])).status_code == 403
    assert client["api"].post("/api/auth/refresh", json={"refresh_token": user["refresh_token"]}).status_code in (401, 403)
    assert client["api"].post("/api/auth/login", json={
...
def test_revoke_sessions_endpoint(client):
    ...
    assert client["api"].get("/api/auth/me", headers=auth_headers(user["access_token"])).status_code == 401
...
def test_usage_summary_aggregates(client):
    ...
    resp = client["admin"].get("/api/admin/usage?days=7", headers=auth_headers(token))
    assert resp.status_code == 200
    body = resp.json()
...
def test_settings_roundtrip_and_audit(client):
    ...
    audit = client["admin"].get("/api/admin/audit", headers=auth_headers(token))
...
def test_key_admin_ops_are_audited(client):
    ...
    created = client["admin"].post("/api/admin/invites", json={}, headers=auth_headers(token))
    invite_id = created.json()["id"]
    client["admin"].delete(f"/api/admin/invites/{invite_id}", headers=auth_headers(token))
    audit = client["admin"].get("/api/admin/audit", headers=auth_headers(token)).json()
...
def test_admin_endpoints_require_auth(client):
    assert client["admin"].get("/api/admin/users").status_code == 401
    assert client["admin"].get("/api/admin/audit").status_code == 401
